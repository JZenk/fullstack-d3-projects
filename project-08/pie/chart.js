async function drawPie() {
  ...
  const colorScale = d3
    ...
    .domain(
      arcs
        .sort((a, b) => a.data.values.length - b.data.values.length)
        .map(d => d.data.key)
    )
    .range(
  ...
  const centeredGroup = bounds
    .append("g")
    .attr(
      "transform",
      `translate(${dimensions.boundedHeight / 2}, ${dimensions.boundedWidth /
        2})`
    );
  ...
  centeredGroup
    ...
    .append("path")
    .attr("fill", d =>
      d.data.key == "other" ? "#dadadd" : colorScale(d.data.key)
    )
    .attr("d", arc)
    .append("title")
    .text(d => d.data.key);

  const iconGroups = centeredGroup
    .selectAll("g")
    .data(arcs)
    .enter()
    .append("g")
